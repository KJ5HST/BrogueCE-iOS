// BrogueCE iOS - Input Handling
// Adapted from Android port
...
boolean process_events() {
    ...
    static SDL_FingerID primary_finger = -1;  // Track primary touch finger
    static int num_fingers = 0;  // Track number of active touches
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_FINGERDOWN:
            num_fingers++;
            // First finger becomes primary
            if (num_fingers == 1) {
                primary_finger = event.tfinger.fingerId;
            } else if (num_fingers == 2) {
                // Second finger - zoom toggle gesture
                zoom_toggled_time = SDL_GetTicks();
                break;  // Don't overwrite existing events
            } else if (num_fingers >= 3) {
                // Third+ finger - ctrl modifier
                if (!ctrl_pressed) {
            ...
            // Only process primary finger
            if (event.tfinger.fingerId != primary_finger) {
                break;  // Don't overwrite existing events
            }
            ...
            if (game_started && dpad_enabled && SDL_PointInRect(&p, &dpad_area)) {
                on_dpad = true;
            ...
            if (smart_zoom && left_panel_smart_zoom && SDL_PointInRect(&p, &left_panel_box) && cursor_x > LEFT_EDGE_WIDTH) {
                if (cursor_y <= ROWS / 2) {  // Simplified sidebar check
                    in_left_panel = true;
        ...
        case SDL_FINGERUP:
            if (num_fingers > 0) num_fingers--;
            // Handle non-primary finger release - don't overwrite existing events
            if (event.tfinger.fingerId != primary_finger) {
                // Check for zoom toggle gesture (quick two-finger tap)
                if (!ctrl_pressed && num_fingers == 1 &&
                    !SDL_TICKS_PASSED(SDL_GetTicks(), zoom_toggled_time + ZOOM_TOGGLED_INTERVAL)) {
                    zoom_toggle = !zoom_toggle;
                    screen_changed = true;
                }
                break;  // Don't set EVENT_ERROR, just skip
            }
            // Reset primary finger when released
            if (num_fingers == 0) {
                primary_finger = -1;
            }
            if (!SDL_TICKS_PASSED(SDL_GetTicks(), zoom_changed_time + ZOOM_CHANGED_INTERVAL)) {
        ...
        case SDL_FINGERMOTION:
            ...
            if (finger_down_time != 0 && SDL_TICKS_PASSED(SDL_GetTicks(), finger_down_time + long_press_interval)) {
                if (on_dpad && allow_dpad_mode_change) {
                    dpad_mode = !dpad_mode;
                    screen_changed = true;
                    pauseForMilliseconds(0, PAUSE_BEHAVIOR_DEFAULT);
                }
                finger_down_time = 0;
            }
            break;
        ...
        case SDL_MOUSEBUTTONDOWN:
            // Handle mouse clicks (iOS Simulator uses these)
            raw_input_x = event.button.x * display_scale;
            raw_input_y = event.button.y * display_scale;
            cursor_x = min(COLS - 1, raw_input_x / cell_w);
            cursor_y = min(ROWS - 1, raw_input_y / cell_h);
            current_event.param1 = cursor_x;
            current_event.param2 = cursor_y;
        ...
        case SDL_MOUSEBUTTONUP:
            // Handle mouse releases (iOS Simulator uses these)
            raw_input_x = event.button.x * display_scale;
            raw_input_y = event.button.y * display_scale;
            cursor_x = min(COLS - 1, raw_input_x / cell_w);
            cursor_y = min(ROWS - 1, raw_input_y / cell_h);
        ...
        case SDL_APP_WILLENTERBACKGROUND:
            // iOS: Save game state when going to background
            break;

        case SDL_APP_DIDENTERFOREGROUND:
            // iOS: Restore rendering when coming back
            screen_changed = true;
            break;
        }
    }
