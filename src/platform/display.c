// BrogueCE iOS - Display/Rendering
...
void draw_glyph(enum displayGlyph c, struct SDL_FRect rect, uint8_t r, uint8_t g, uint8_t b) {
    ...
    if (font == NULL) {
        return; // Font not loaded, can't render
    }
    ...
    if (lc->c == NULL) {
        ...
        if (text == NULL) {
            // Glyph couldn't be rendered - use a fallback or skip
            // Try to render a simple placeholder character instead
            text = TTF_RenderGlyph_Blended(font, '?', fc);
            if (text == NULL) {
...
void init_glyph_index_table() {
    ...
    // G_EASY removed in newer BrogueCE
    ...
    // G_GOOD_ITEM removed in newer BrogueCE
    init_glyph_index(G_BAD_MAGIC, 128 + 12, 321)
    // G_BAD_ITEM removed in newer BrogueCE
    init_glyph_index(G_AMULET, 128 + 5, 323)
    // G_AMULET_ITEM removed in newer BrogueCE
    ...
    // G_HOLE removed in newer BrogueCE
    init_glyph_index(G_FIRE, 128 + 3, 330)
    init_glyph_index(G_TORCH, '#', 331)
    // G_FALLEN_TORCH removed in newer BrogueCE
    ...
    // G_SACRED_GLYPH, G_GLOWING_GLYPH removed in newer BrogueCE
    ...
    // G_CRYSTAL_WALL removed in newer BrogueCE
    ...
    // Various trap subtypes removed in newer BrogueCE - they all use G_TRAP now
...
boolean init_font() {
    // On iOS, assets are in the app bundle - use SDL_GetBasePath() to find them
    char *base_path = SDL_GetBasePath();
    char font_path[PATH_MAX];

    if (base_path) {
        snprintf(font_path, PATH_MAX, "%sassets/default.ttf", base_path);
        SDL_free(base_path);
    } else {
        // Fallback
        strncpy(font_path, "assets/default.ttf", PATH_MAX);
    }
...
boolean smart_zoom_allowed() {
    ...
    // Simplified: only allow zoom when actually in game
    return game_started && rogue.depthLevel > 0;
}
...
void draw_screen() {
    if (screen_changed) {
        ...
        // Draw D-pad if enabled and in game
        if (game_started && dpad_enabled) {
...
void refresh_animations(boolean colorsDance) {
    ...
    if (graphicsEnabled && tiles_animation) {
        ...
        // Animation refresh removed - BrogueCE doesn't have needsUpdate in cellDisplayBuffer
        // For now, just force a redraw when tiles flip
